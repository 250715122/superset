...
本文档面向**生产 / 准生产**场景，给出一套把本仓库（apache/superset 的 fork，基线 6.0）以「**构建机出 wheel → 远程云服务器 `pip install wheel`**」的方式部署上线的完整步骤。这是 Apache Superset 官方 PyPI 安装方式（`pip install apache-superset`）应用到自有 fork 时的**完整等价做法**——你自己当一回 release manager，给自己的 fork 打一份 wheel。
...
> **前置**：请先在云机上跑完[第六节](#六云服务器装系统依赖每台云机一次性)和[第七节](#七云服务器venv--pip-install-wheel)开头的目录创建步骤，确保 `/opt/superset/` 已存在且当前用户有写权限，然后再回来执行下面的 scp。这样可以直接把所有文件放到永久位置，省掉中转步骤。

```bash
# 1. wheel 本体（直接放到云端工作目录 /opt/superset/）
scp /opt/superset-build/dist/apache_superset-*.whl <user>@<cloud-host>:/opt/superset/

# 2. build commit 记录
scp /opt/superset-build/dist/BUILD_COMMIT.txt <user>@<cloud-host>:/opt/superset/
...
scp /path/to/your/superset_config.py <user>@<cloud-host>:/opt/superset/
```
...
>     scp /opt/superset-build/dist/*.whl                   ${host}:/opt/superset/
>     scp /opt/superset-build/requirements/base.txt        ${host}:/opt/superset/base-constraints.txt
> done
> ```
...
```bash
cp /opt/superset/BUILD_COMMIT.txt /opt/superset/.deployed_commit
...
```bash
# wheel 已经装进 venv，原始文件可以删
# 注意：base-constraints.txt 留着，升级和补装依赖时还要用
rm -f /opt/superset/apache_superset-*.whl /opt/superset/BUILD_COMMIT.txt
```
...
- `ImportError`：检查自定义 SecurityManager 是否引用了不存在的 6.0 API
...
| 修源码 | 直接编辑 `/opt/superset-6.0-bluetti/superset/*.py` | 在构建机的 git 仓库改，**云上没有源码** |
